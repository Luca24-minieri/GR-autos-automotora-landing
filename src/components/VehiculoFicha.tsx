...
import { type Vehiculo, formatPrecio, incrementClicks } from "@/lib/vehicles";
...
function SpecTable({ v }: { v: Vehiculo }) {
  const specs = [
    ...
    ...(v.potencia ? [["Potencia", v.potencia]] : []),
    ["Tracción", v.traccion],
    ["Color", v.colorExterior],
    ...(v.colorInterior ? [["Color interior", v.colorInterior]] : []),
    ["Puertas", String(v.puertas)],
    ...(v.numeroDuenos > 0 ? [["N° de dueños", String(v.numeroDuenos)]] : []),
  ];
  return (
...
interface VehiculoFichaProps {
  vehiculo: Vehiculo;
  todosVehiculos: Vehiculo[];
}

export default function VehiculoFicha({ vehiculo: v, todosVehiculos }: VehiculoFichaProps) {
  const [contactPref, setContactPref] = useState("whatsapp");
  const [copied, setCopied] = useState(false);
  const [generatingPdf, setGeneratingPdf] = useState(false);

  // Increment clicks on mount
  useEffect(() => {
    incrementClicks(v.id);
  }, [v.id]);
  ...
  const handlePdf = useCallback(async () => {
    ...
    try {
      ...
      pdf.text(`${v.marca} ${v.modelo} ${v.ano}`, 15, 40);
  ...
  const similares = todosVehiculos
    .filter((x) => x.marca === v.marca && x.id !== v.id)
    .slice(0, 3);

  const badges = getAutoBadges(v);

  const consultaMsg = `Hola, estoy interesado/a en el ${v.marca} ${v.modelo} ${v.ano} publicado a ${formatPrecio(v.precio)}. ¿Está disponible?`;
  ...
  return (
    <main className="min-h-screen bg-background pt-24 pb-16">
      ...
      {/* Overlay para vehículo vendido */}
      {v.estado === "vendido" && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="mx-4 rounded-2xl border border-white/10 bg-surface p-8 text-center shadow-2xl">
            <p className="font-display text-2xl font-bold text-white">
              Este vehículo ya fue vendido
            </p>
            <p className="mt-2 text-sm text-muted-foreground">
              Pero tenemos más opciones para ti en nuestro catálogo.
            </p>
            <Link
              href="/vehiculos"
              className="btn-press mt-6 inline-block rounded-full bg-gold px-8 py-3 font-medium text-white transition-[background-color] duration-200 hover:bg-gold-hover"
            >
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_400px]">
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-lg border border-white/[0.06] bg-surface p-6">
              ...
              >
                {v.estado === "disponible" ? "Disponible" : v.estado === "reservado" ? "Reservado" : "Vendido"}
              </span>

              <h2 className="mt-2 font-display text-xl font-semibold text-white">
                {v.marca} {v.modelo} {v.ano}
              </h2>
              {v.version && <p className="text-sm text-muted-foreground">{v.version}</p>}
